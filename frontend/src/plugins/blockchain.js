...
const TransferTransaction = {
  ...
  fields: [
    ...
    { name: 'to_second', type: Exonum.PublicKey },
    { name: 'to_third', type: Exonum.PublicKey },
    { name: 'amount', type: Exonum.Uint64 },
    { name: 'seed', type: Exonum.Uint64 },
    // { name: 'time', type: Exonum.Uint64 },
    { name: 'tx_time', type: Exonum.String },
  ]
}
const IssueTransaction = {
  ...
  fields: [
    ...
    { name: 'amount', type: Exonum.Uint64 },
    { name: 'amount_second', type: Exonum.Uint64 },
    { name: 'amount_third', type: Exonum.Uint64 },
    { name: 'seed', type: Exonum.Uint64 }
  ]
}
...
module.exports = {
  install(Vue) {
    Vue.prototype.$blockchain = {
      ...
      addFunds(keyPair, amountToAdd, amountToAdd2, amountToAdd3) {
        const TxIssue = getTransaction(TX_ISSUE_ID)

        const data = {
          pub_key: keyPair.publicKey,
          amount: amountToAdd.toString(),
          amount_second: amountToAdd2.toString(),
          amount_third: amountToAdd3.toString(),
      ...
      transfer(keyPair, receiver, receiver_second, receiver_third, amountToTransfer) {
        const TxTransfer = getTransaction(TX_TRANSFER_ID)

        const data = {
          from: keyPair.publicKey,
          to: receiver,
          to_second: receiver_second,
          to_third: receiver_third,
          amount: amountToTransfer,
          seed: Exonum.randomUint64(),
          tx_time: new Date().getTime().toString(),
        }
