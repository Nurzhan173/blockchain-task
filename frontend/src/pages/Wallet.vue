                  ...
                  <div class="col-sm-12">
                    <router-link :to="{ name: 'transaction', params: { hash: transaction.hash } }">
                      ...
                      <span v-else-if="transaction.message_id === 1">
                        <strong v-numeral="transaction.body.amount"/>,     
                        <strong v-numeral="transaction.body.amount_second"/>,   
                        <strong v-numeral="transaction.body.amount_third"/> funds added
                      </span>
            ...
            <div class="card-body">
              <form @submit.prevent="addFunds">
                <div class="form-group">
                  ...
                  <div v-for="variant in variantsSecond" class="form-check form-check-inline">
                    <input :id="variant.id" :value="variant.amount" :checked="amountToAddSeconds == variant.amount" v-model="amountToAddSeconds" class="form-check-input" type="radio">
                    <label :for="variant.id" class="form-check-label">${{ variant.amount }}</label>
                  </div><br>
                  <div v-for="variant in variantsThird" class="form-check form-check-inline">
                    <input :id="variant.id" :value="variant.amount" :checked="amountToAddThird == variant.amount" v-model="amountToAddThird" class="form-check-input" type="radio">
                    <label :for="variant.id" class="form-check-label">${{ variant.amount }}</label>
                  </div>
                </div>
            ...
            <div class="card-body">
              <form @submit.prevent="transfer">
                <div class="form-group">
                  ...
                  <input v-model="receiver_second" type="text" class="form-control" placeholder="Enter public key" required>
                  <input v-model="receiver_third" type="text" class="form-control" placeholder="Enter public key" required>
                </div>
                <div class="form-group">
                  <label>Amount:</label>
  ...
  module.exports = {
    ...
    methods: {
      ...
      async addFunds() {
        ...
        try {
          const data = await this.$blockchain.addFunds(this.keyPair, this.amountToAdd, this.amountToAddSeconds, this.amountToAddThird)
          this.balance = data.wallet.balance
          this.transactions = data.transactions
          this.isSpinnerVisible = false
      ...
      async transfer() {
        ...
        try {
          const data = await this.$blockchain.transfer(this.keyPair, this.receiver, this.receiver_second, this.receiver_third, this.amountToTransfer)
          this.balance = data.wallet.balance
          this.transactions = data.transactions
          this.isSpinnerVisible = false
